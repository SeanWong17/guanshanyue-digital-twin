...
renderer.setAnimationLoop(t=>{sunlight?.tick(t);controls.update();camera.updateMatrixWorld();for(const {el,p} of labels){const v=p.clone().project(camera);el.style.display=!showLabels||v.z>1||v.z< -1?'none':'block';el.style.left=`${(v.x+1)*host.clientWidth/2}px`;el.style.top=`${(1-v.y)*host.clientHeight/2}px`;}renderer.render(scene,camera);});
window.__community={scene,camera,controls,get model(){return model;},data,sunlight};
